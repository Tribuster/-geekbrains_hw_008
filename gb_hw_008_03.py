...
class TypeCheck:
    ...
    def check_value(self):
        ...
        while True:
            try:
                try:
                    user_val = int(input('Введите целое число: '))
                    ex = input(f'Всё отлично, добавляем "{user_val}" в список. Для продолжения "Enter", для завершения "n": ').lower()
                ...
                ex = input(f'Вы ввели не число! Для продолжения "Enter", для завершения "n": ').lower()
                if ex == 'n':
                    print(self.my_list)
                    break
                else:
